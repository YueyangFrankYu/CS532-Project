...
streaming_df = spark.readStream.option("delimiter", "\t").schema(schema).option("maxFilesPerTrigger", 1).csv("./output_folder", header=True)
...
original_data = pd.read_csv('./data_test/part-00000-29b0fbf4-8735-4a95-88cd-77da64268f21-c000.csv',error_bad_lines=False)  # Replace with the actual path
...
output_directory = './output_folder_batchsize_500'
...
# Load model from file
model = PipelineModel.load("/content/drive/MyDrive/Colab Notebooks/model")
nltk.download('stopwords')
nltk.download('wordnet')
# Initialize Spark Session
spark = SparkSession.builder.appName("TweetStreaming").getOrCreate()
...
#  Streaming from a directory
streaming_df = spark.readStream.option("delimiter", "\t").schema(schema).option("maxFilesPerTrigger", 1).csv("/content/drive/MyDrive/output_folder_batchsize_500", header=True)
